...
def plot_graph(G, show_NPD_labels=True, show_SE_labels=False, show_NP_labels=True):
    ...
    plt.title("Neuronal Packet Domains (NPDs) as Projectors and Knowledge Domain (KDs) as Content Generators\n(A static illustration at a given time snapshot)", fontsize=14, fontweight='bold')
    #plt.text(0.5, 1.02, "(A static illustration at a given time snapshot)", fontsize=12, ha='center', transform=plt.gca().transAxes)    
    plt.tight_layout()
    
    # Add legend
    legend_elements = [
        plt.Line2D([0], [0], marker='s', color='grey', markerfacecolor='grey', alpha=0.7, markersize=10, label='Superordinate Ensemble of non-local Heterachical KDs'),
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='none', markeredgecolor='grey', markersize=10, label='Localized KD as a Superordinate Ensemble of NPDs'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='grey', markeredgecolor='grey', alpha=0.7, markersize=10, label='NPD'),
    ]
...
domain_config = [
    ...
    {'domain_color': 'green', 'num_ensembles': 3, 'num_NPs': (4, 7)},
    {'domain_color': 'red', 'num_ensembles': 4, 'num_NPs': (3, 8)},
    {'domain_color': 'purple', 'num_ensembles': 3, 'num_NPs': (3, 6)},
